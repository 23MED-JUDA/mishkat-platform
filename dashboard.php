...
$userQuery = $conn->query("SELECT u.*, r.name AS role FROM users u JOIN roles r ON u.role_id = r.id WHERE u.id = $uid");
...
$allPossibleLinks = array_merge($links, [
    ['name' => 'القرآن الكريم', 'page' => 'quran'],
    ['name' => 'الأحاديث النبوية', 'page' => 'hadith'],
    ['name' => 'السيرة النبوية', 'page' => 'seerah'],
    ['name' => 'السبحة الرقمية', 'page' => 'tasbih'],
    ['name' => 'اختبارات مشكاة', 'page' => 'quiz']
]);
...
<body class="transition-colors duration-500 overflow-x-hidden">
    ...
    <script>
        ...
        /* إغلاق بالسحب إلى اليمين (موبايل) */
        let touchStartX = 0;
        ...
        /* فتح بالسحب إلى اليسار من الطرف */
