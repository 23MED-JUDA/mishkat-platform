...
$parentsCount = $conn->query("SELECT COUNT(*) as c FROM users WHERE role_id=4")->fetch_assoc()['c'];
$users = $conn->query("SELECT u.id, u.name, u.email, '' AS phone, r.name AS role, u.status, u.created_at FROM users u JOIN roles r ON u.role_id = r.id ORDER BY u.created_at DESC");
            ...
            <tbody class="divide-y divide-gray-50" id="usersTable">
                ...
                <tr class="user-row hover:bg-gray-50/50 transition-all" data-name="<?php echo htmlspecialchars($u['name']); ?>" data-role="<?php echo $u['role']; ?>" data-id="<?php echo $u['id']; ?>">
                    ...
                    <td class="px-6 py-4"><div class="flex gap-1">
                        <button onclick="deleteUser(<?php echo $u['id']; ?>)" class="p-2 bg-gray-50 rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-600 transition-all" title="حذف الحساب">
                            <span class="material-icons-outlined text-sm">delete</span>
                        </button>
                        <?php if (($u['role'] === 'teacher' || $u['role'] === 'معلم') && $u['status'] !== 'active'): ?>
                        <button onclick="approveTeacher(<?php echo $u['id']; ?>)" class="p-2 bg-green-50 rounded-lg text-green-600 hover:bg-green-100 transition-all" title="قبول المعلم">
                            <span class="material-icons-outlined text-sm">check_circle</span>
                        </button>
                        <button onclick="rejectTeacher(<?php echo $u['id']; ?>)" class="p-2 bg-red-50 rounded-lg text-red-600 hover:bg-red-100 transition-all" title="رفض المعلم">
...
function approveTeacher(id) {
    apiCall('approve_teacher',{user_id:id}).then(res => {
        if(res.success) { showToast('تم اعتماد المعلم'); setTimeout(() => location.reload(),500); }
        else showToast(res.message||'خطأ في الاعتماد','error');
    });
}
function rejectTeacher(id) {
    apiCall('reject_teacher',{user_id:id}).then(res => {
        if(res.success) { showToast('تم رفض المعلم'); setTimeout(() => location.reload(),500); }
        else showToast(res.message||'خطأ في الرفض','error');
    });
}
