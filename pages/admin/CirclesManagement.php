...
if ($role === 'teacher') {
    ...
    $circles = $conn->query("SELECT ci.*, 'active' AS status, u.name as teacher_name, (SELECT COUNT(*) FROM halaqa_enrollments WHERE halaqa_id=ci.id) as current_students FROM halaqat ci LEFT JOIN teachers t ON ci.teacher_id=t.id LEFT JOIN users u ON t.user_id=u.id WHERE ci.teacher_id=$teacher_id ORDER BY ci.id DESC");
    $teachers = $conn->query("SELECT id, name FROM users WHERE id=$uid");
} else {
    $circles = $conn->query("SELECT ci.*, 'active' AS status, u.name as teacher_name, (SELECT COUNT(*) FROM halaqa_enrollments WHERE halaqa_id=ci.id) as current_students FROM halaqat ci LEFT JOIN teachers t ON ci.teacher_id=t.id LEFT JOIN users u ON t.user_id=u.id ORDER BY ci.id DESC");
    $teachers = $conn->query("SELECT u.id, u.name FROM users u JOIN roles r ON u.role_id = r.id WHERE r.name='teacher' ORDER BY u.name");
}
?>
