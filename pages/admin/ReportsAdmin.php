...
$stats = [
    ...
    'courses' => $conn->query("SELECT COUNT(*) as c FROM learning_paths")->fetch_assoc()['c'],
    'circles' => $conn->query("SELECT COUNT(*) as c FROM halaqat")->fetch_assoc()['c'],
];
...
$recentLogs = $conn->query("SELECT u.name, r.name AS role, p.amount, p.created_at FROM payments p JOIN users u ON p.user_id = u.id JOIN roles r ON u.role_id = r.id ORDER BY p.created_at DESC LIMIT 10");
?>
