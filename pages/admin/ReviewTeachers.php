<?php
// Admin Review Teachers - Enhanced for Approval Flow
$teachersQuery = "SELECT u.id, u.name, u.email, '' AS phone, u.status, u.created_at, u.gender, '' AS location,
    t.specialization AS specialty, t.experience_years, t.cv_file AS cv_url, t.bio, 
    IFNULL((SELECT AVG(rating) FROM teacher_ratings WHERE teacher_id = t.id), 5.0) AS rating
    FROM users u 
    LEFT JOIN teachers t ON u.id = t.user_id 
    JOIN roles r ON u.role_id = r.id 
    WHERE r.name = 'teacher' 
    ORDER BY FIELD(u.status, 'pending', 'active', 'inactive'), u.created_at DESC";
$teachers = $conn->query($teachersQuery);

if (!$teachers) {
    echo "<div class='p-10 luxury-card rounded-3xl border border-red-100 bg-red-50 text-red-700 font-bold'>خطأ في قاعدة البيانات: " . $conn->error . "</div>";
    return;
}
?>
<div class="space-y-6 animate-fadeIn" dir="rtl">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-3xl font-black text-gray-900">مراجعة المعلمين</h2>
            <p class="text-sm text-gray-500 font-bold mt-1">إدارة طلبات الانضمام والموافقة على حسابات المعلمين</p>
        </div>
        <div class="flex gap-2 items-center">
            <?php
                $pendingCount = $conn->query("SELECT COUNT(*) as c FROM users u JOIN roles r ON u.role_id = r.id WHERE r.name = 'teacher' AND u.status = 'pending'")->fetch_assoc()['c'] ?? 0;
            ?>
            <span class="px-4 py-2 bg-amber-50 text-amber-700 rounded-[1.5rem] text-xs font-black border border-amber-200">
                <?php echo $pendingCount; ?> بانتظار المراجعة
            </span>
            <?php if($pendingCount > 0): ?>
            <button onclick="approveAllTeachers()" class="px-5 py-2 bg-mishkat-green-600 text-white rounded-[1.5rem] text-xs font-black hover:bg-mishkat-green-700 transition-all flex items-center gap-2 shadow-lg shadow-mishkat-green-100">
                <span class="material-icons-outlined text-sm">done_all</span>
                قبول الكل (<?php echo $pendingCount; ?>)
            </button>
            <?php endif; ?>
        </div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php if($teachers->num_rows === 0): ?>
            <div class="col-span-full p-20 text-center luxury-card rounded-3xl border border-dashed border-gray-200">
                <span class="material-icons-outlined text-6xl text-gray-200 mb-4">person_search</span>
                <p class="text-gray-400 font-bold">لا توجد طلبات انضمام حالياً</p>
            </div>
        <?php endif; ?>

        <?php while($t = $teachers->fetch_assoc()): 
            $isPending = ($t['status'] === 'pending');
            $isActive  = ($t['status'] === 'active');
            $isInactive= ($t['status'] === 'inactive');
        ?>
        <div class="luxury-card rounded-[2.5rem] shadow-sm border <?php
            if($isPending)  echo 'border-amber-200 bg-amber-50/20';
            elseif($isActive) echo 'border-emerald-100';
            else echo 'border-red-100 bg-red-50/10';
        ?> p-8 hover:shadow-xl transition-all group relative overflow-hidden">
            
            <!-- Status Badge -->
            <?php if($isPending): ?>
            <div class="absolute top-0 left-0 px-4 py-1 bg-amber-500 text-white text-[10px] font-black rounded-br-2xl flex items-center gap-1">
                <span class="material-icons-outlined text-xs">schedule</span> قيد المراجعة
            </div>
            <?php elseif($isActive): ?>
            <div class="absolute top-0 left-0 px-4 py-1 bg-emerald-500 text-white text-[10px] font-black rounded-br-2xl flex items-center gap-1">
                <span class="material-icons-outlined text-xs">check_circle</span> نشط
            </div>
            <?php else: ?>
            <div class="absolute top-0 left-0 px-4 py-1 bg-red-400 text-white text-[10px] font-black rounded-br-2xl flex items-center gap-1">
                <span class="material-icons-outlined text-xs">block</span> موقوف
            </div>
            <?php endif; ?>

            <!-- Teacher Info -->
            <div class="flex items-start justify-between mb-6 mt-4">
                <div class="flex items-center gap-4">
                    <div class="w-16 h-16 rounded-[2rem] <?php
                        if($isPending)  echo 'bg-amber-500';
                        elseif($isActive) echo 'bg-mishkat-green-700';
                        else echo 'bg-gray-400';
                    ?> text-white flex items-center justify-center text-3xl font-black shadow-lg group-hover:rotate-6 transition-transform">
                        <?php echo mb_substr($t['name'], 0, 1, 'UTF-8'); ?>
                    </div>
                    <div>
                        <h3 class="font-black text-gray-900 text-lg"><?php echo htmlspecialchars($t['name']); ?></h3>
                        <p class="text-xs text-mishkat-green-600 font-bold"><?php echo htmlspecialchars($t['specialty'] ?? 'تخصص غير محدد'); ?></p>
                        <p class="text-[10px] text-gray-400 font-bold mt-1"><?php echo htmlspecialchars($t['email']); ?></p>
                    </div>
                </div>
            </div>

            <!-- Stats -->
            <div class="grid grid-cols-2 gap-3 mb-6">
                <div class="p-3 bg-gray-50 rounded-2xl border border-gray-100">
                    <p class="text-[10px] text-gray-400 font-bold mb-1">الخبرة</p>
                    <p class="text-sm font-black text-gray-700"><?php echo $t['experience_years'] ?? 0; ?> سنوات</p>
                </div>
                <div class="p-3 bg-gray-50 rounded-2xl border border-gray-100">
                    <p class="text-[10px] text-gray-400 font-bold mb-1">التقييم</p>
                    <p class="text-sm font-black text-gray-700 flex items-center gap-1">
                        <span class="material-icons-outlined text-xs text-yellow-500">star</span>
                        <?php echo number_format($t['rating'] ?? 5, 1); ?>
                    </p>
                </div>
            </div>

            <?php if($t['cv_url']): ?>
            <a href="<?php echo htmlspecialchars($t['cv_url']); ?>" target="_blank"
               class="w-full py-3 mb-4 bg-mishkat-green-50 text-mishkat-green-700 rounded-[1.5rem] text-xs font-black flex items-center justify-center gap-2 hover:bg-mishkat-green-100 transition-all border border-mishkat-green-100">
                <span class="material-icons-outlined text-sm">description</span> عرض السيرة الذاتية
            </a>
            <?php endif; ?>

            <!-- Action Buttons -->
            <div class="flex gap-2 mt-2">
                <?php if($isPending): ?>
                    <!-- Pending: Approve + Reject -->
                    <button onclick="approveTeacher(<?php echo $t['id']; ?>)"
                        class="flex-1 py-3 bg-emerald-600 text-white rounded-2xl text-xs font-black hover:bg-emerald-700 transition-all flex items-center justify-center gap-1 shadow-md">
                        <span class="material-icons-outlined text-sm">check_circle</span> قبول
                    </button>
                    <button onclick="rejectTeacher(<?php echo $t['id']; ?>)"
                        class="flex-1 py-3 bg-red-50 text-red-600 border border-red-100 rounded-2xl text-xs font-black hover:bg-red-100 transition-all flex items-center justify-center gap-1">
                        <span class="material-icons-outlined text-sm">cancel</span> رفض
                    </button>

                <?php elseif($isActive): ?>
                    <!-- Active: Suspend + Delete -->
                    <button onclick="toggleTeacherStatus(<?php echo $t['id']; ?>, 'inactive')"
                        class="flex-1 py-3 bg-amber-50 text-amber-700 border border-amber-200 rounded-2xl text-xs font-black hover:bg-amber-100 transition-all flex items-center justify-center gap-1">
                        <span class="material-icons-outlined text-sm">pause_circle</span> تعليق
                    </button>
                    <button onclick="deleteTeacher(<?php echo $t['id']; ?>)"
                        class="px-4 py-3 bg-gray-50 text-gray-400 rounded-2xl hover:bg-red-50 hover:text-red-500 transition-all border border-gray-100">
                        <span class="material-icons-outlined text-sm">delete</span>
                    </button>

                <?php else: ?>
                    <!-- Inactive: Activate + Delete -->
                    <button onclick="toggleTeacherStatus(<?php echo $t['id']; ?>, 'active')"
                        class="flex-1 py-3 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-2xl text-xs font-black hover:bg-emerald-100 transition-all flex items-center justify-center gap-1">
                        <span class="material-icons-outlined text-sm">play_circle</span> تنشيط
                    </button>
                    <button onclick="deleteTeacher(<?php echo $t['id']; ?>)"
                        class="px-4 py-3 bg-gray-50 text-gray-400 rounded-2xl hover:bg-red-50 hover:text-red-500 transition-all border border-gray-100">
                        <span class="material-icons-outlined text-sm">delete</span>
                    </button>
                <?php endif; ?>
            </div>
        </div>
        <?php endwhile; ?>
    </div>
</div>

<script>
function approveTeacher(id) {
    apiCall('approve_teacher', {user_id: id}).then(res => {
        if(res.success) {
            showToast('تم قبول المعلم وتفعيل حسابه بنجاح', 'success');
            setTimeout(() => location.reload(), 800);
        } else {
            showToast(res.message || 'خطأ في تنفيذ الإجراء', 'error');
        }
    });
}

function rejectTeacher(id) {
    confirmDialog('هل أنت متأكد من رفض طلب انضمام هذا المعلم؟').then(ok => {
        if(ok) {
            apiCall('reject_teacher', {user_id: id}).then(res => {
                if(res.success) {
                    showToast('تم رفض طلب المعلم');
                    setTimeout(() => location.reload(), 800);
                } else {
                    showToast(res.message || 'خطأ في تنفيذ الإجراء', 'error');
                }
            });
        }
    });
}

function toggleTeacherStatus(id, newStatus) {
    apiCall('toggle_user_status', {user_id: id, force_status: newStatus}).then(res => {
        if(res.success) {
            showToast(newStatus === 'active' ? 'تم تنشيط الحساب' : 'تم تعليق الحساب');
            setTimeout(() => location.reload(), 800);
        } else {
            showToast(res.message || 'خطأ في تنفيذ الإجراء', 'error');
        }
    });
}

function deleteTeacher(id) {
    confirmDialog('هل أنت متأكد من حذف هذا الحساب نهائياً؟').then(ok => {
        if(ok) apiCall('delete_user', {user_id: id}).then(res => {
            if(res.success) {
                showToast('تم حذف المعلم');
                setTimeout(() => location.reload(), 800);
            }
        });
    });
}
function approveAllTeachers() {
    confirmDialog('هل أنت متأكد من قبول جميع المعلمين المنتظرين دفعة واحدة؟').then(ok => {
        if(ok) {
            apiCall('approve_all_teachers', {}).then(res => {
                if(res.success) {
                    showToast(res.message || 'تم قبول جميع المعلمين بنجاح', 'success');
                    setTimeout(() => location.reload(), 800);
                } else {
                    showToast(res.message || 'خطأ في تنفيذ الإجراء', 'error');
                }
            });
        }
    });
}
</script>
