...
$students = $conn->query("SELECT u.id, u.name, u.email, '' AS phone, u.status, u.created_at,
    ...
    LEFT JOIN student_paths sp ON s.id=sp.student_id
    LEFT JOIN learning_paths c ON sp.path_id=c.id 
    ORDER BY u.created_at DESC");
$totalStudents = $conn->query("SELECT COUNT(*) as c FROM students")->fetch_assoc()['c'];
$activeStudents = $conn->query("SELECT COUNT(*) as c FROM students s JOIN users u ON s.user_id=u.id WHERE u.status='active'")->fetch_assoc()['c'];
?>
