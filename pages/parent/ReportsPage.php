...
<div class="space-y-6 animate-fadeIn" dir="rtl">
    ...
    <?php foreach($childrenArr as $child): 
        $cid = $child['id'];
        $studentRow = $conn->query("SELECT id FROM students WHERE user_id=$cid")->fetch_assoc();
        ...
        if ($studentId > 0) {
            ...
            $completedTasks = $conn->query("SELECT COUNT(*) as c FROM homework_submissions WHERE student_id = $studentId AND status IN ('completed', 'graded')")->fetch_assoc()['c'] ?? 0;
        }
        ...
        $enrollments = $conn->query("SELECT 0 AS progress, lp.name AS title, 'emerald' AS color FROM student_paths sp JOIN learning_paths lp ON sp.path_id=lp.id JOIN students s ON sp.student_id=s.id WHERE s.user_id=$cid");
    ?>
