...
$enrollments = $conn->query("SELECT 0 AS progress, lp.name AS title, 'emerald' AS color FROM student_paths sp JOIN learning_paths lp ON sp.path_id=lp.id WHERE sp.student_id=$studentId");
$completedTasks = $conn->query("SELECT COUNT(*) as c FROM homework_submissions WHERE student_id=$studentId AND (status='completed' OR status='graded')")->fetch_assoc()['c'] ?? 0;
